...
func TestProtectDomainsAndFiles(t *testing.T) {
	tests := []struct {
		...
		{"filenameWithNumberedItems", "File name:\n1.MTL Daily Report LINE_BK_Broker\n2.KLINE Broker Daily Transaction\n3.KBank Daily Insurance Pay", "File name:\n`1.MTL Daily Report LINE_BK_Broker`\n`2.KLINE Broker Daily Transaction`\n`3.KBank Daily Insurance Pay`"},
		{"filenameWithPunct", "check config.json.", "check `config.json`."},
